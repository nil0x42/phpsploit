...
def loads(data, charset='utf-8', errors='strict', decode_strings=False,
    ...
    """
    result = load(StringIO(data), charset, errors, decode_strings,
                object_hook, array_hook)

    # <PATCH>
    # MY PATCH TO CONVERT NUMERIC DICTS INTO LISTS: (patch by vAckSyn) #
    # {0:'a',1:'b'} ==> ['a','b']
    def patch_dict2list(var):
        if type(var).__name__ == 'dict':
            if var.keys() == range(0,len(var)):
                var = [var[x] for x in var]
        if type(var).__name__ == 'dict':
            for x in var:
                var[x] = patch_dict2list(var[x])
        if type(var).__name__ == 'list':
            for x in range(0,len(var)):
                var[x] = patch_dict2list(var[x])
        return(var)

    result = patch_dict2list(result)
    # </PATCH>

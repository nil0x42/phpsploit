...
<p>Lookahead to disallow matching with the given parse expression.  
  <code>NotAny</code> does *not* advance the parsing position within the 
  input string, it only verifies that the specified parse expression does 
  *not* match at the current position.  Also, <code>NotAny</code> does 
  *not* skip over leading whitespace. <code>NotAny</code> always returns a 
  null token list.  May be constructed using the '~' operator.</p>

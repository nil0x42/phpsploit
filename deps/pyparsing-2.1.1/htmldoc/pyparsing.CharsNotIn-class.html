...
<p>Token for matching words composed of characters *not* in a given set. 
  ...
  minimum, maximum, and/or exact length.  The default value for 
  <code>min</code> is 1 (a minimum value &lt; 1 is not valid); the default 
  values for <code>max</code> and <code>exact</code> are 0, meaning no 
  maximum or exact length restriction.</p>

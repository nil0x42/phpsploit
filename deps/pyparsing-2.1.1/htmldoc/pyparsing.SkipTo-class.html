...
<p>Token for skipping over all undefined text until the matched 
  ...
  <ul>
    ...
    <li>
      include - (default=False) if True, the target expression is also 
      parsed (the skipped text and target expression are returned as a 
      2-element list).
    </li>
    <li>
      ignore - (default=None) used to define grammars (typically quoted 
      strings and comments) that might contain false matches to the target 
      expression
    </li>
    <li>
      failOn - (default=None) define expressions that are not allowed to be
      included in the skipped test; if found before the target expression 
      is found, the SkipTo is not a match
    </li>
  </ul>
